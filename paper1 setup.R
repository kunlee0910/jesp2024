...
library(readxl)
dataset60 <- read_excel("Data/masterdata.xlsx")
#dataset60 <- dataset60[dataset60$country!="USA",] # analysis without USA
...
countrylist <- unique(dataset60$country)
...
#dataset60$penregime[dataset60$country=="GRC" & dataset60$year>2014] <- 1
dataset60$penregime[dataset60$country=="CZE" | dataset60$country=="USA" | dataset60$country=="CHE" |
...
# Alternative regime variable for robustness checks: Sweden in Mixed regime
dataset60$penregime1 <- 0
#dataset60$penregime1[dataset60$country=="GRC" & dataset60$year>2014] <- 1
...
#dataset60$penexppc2 <- 1000 * 
  #(dataset60$penexp + dataset60$earlyret) * dataset60$gdp / (dataset60$ppp * dataset60$cpi * dataset60$oldpop)

# unemployment benefit spending per working age population
#dataset60$unempexppc <- 1000 * dataset60$unempexp * dataset60$gdp / (dataset60$ppp * dataset60$cpi * dataset60$workpop)
#dataset60 <- dataset60 %>% relocate(unempexppc, .before=unempexp)

# incapacity benefit spending per 50+ population
#dataset60$incapcashpc <- 1000 * dataset60$incapcash * dataset60$gdp / (dataset60$ppp * dataset60$cpi * dataset60$pop50ov)
#dataset60 <- dataset60 %>% relocate(incapcashpc, .before=incapcash)
...
# creating lag panel
#dataset60$emplag <- lagpanel(dataset60$emp, dataset60$group, dataset60$year, 1)
#dataset60 <- dataset60 %>% relocate(emplag, .after=emp)
#dataset65$emplag <- lagpanel(dataset65$emp, dataset65$group, dataset65$year, 1)
#dataset65 <- dataset65 %>% relocate(emplag, .after=emp)

#dataset60$emplag2 <- lagpanel(dataset60$emp, dataset60$group, dataset60$year, 2)
#dataset60 <- dataset60 %>% relocate(emplag2, .after=emplag)
#dataset65$emplag2 <- lagpanel(dataset65$emp, dataset65$group, dataset65$year, 2)
#dataset65 <- dataset65 %>% relocate(emplag2, .after=emplag)

# create differenced variables
#dataset60$empdiff <- dataset60$emp-dataset60$emplag
#dataset60 <- dataset60 %>% relocate(empdiff, .after=emplag2)
#dataset60$empdifflag <- lagpanel(dataset60$empdiff, dataset60$group, dataset60$year, 1)
#dataset60 <- dataset60 %>% relocate(empdifflag, .after=empdiff)

#dataset65$empdiff <- dataset65$emp-dataset65$emplag
#dataset65 <- dataset65 %>% relocate(empdiff, .after=emplag2)
#dataset65$empdifflag <- lagpanel(dataset65$empdiff, dataset65$group, dataset65$year, 1)
#dataset65 <- dataset65 %>% relocate(empdifflag, .after=empdiff)
...
gdum <- makeFEdummies(dataset60$group)
ydum <- makeFEdummies(dataset60$year)[,-1]
mgdum <- makeFEdummies(mdata60$group)
fgdum <- makeFEdummies(fdata60$group)
mydum <- makeFEdummies(mdata60$year)[,-1]
fydum <- makeFEdummies(fdata60$year)[,-1]

gdumpov <- makeFEdummies(datasetpov$group)
ydumpov <- makeFEdummies(datasetpov$year)[,-1]
mgdumpov <- makeFEdummies(mdatapov$group)
fgdumpov <- makeFEdummies(fdatapov$group)
mydumpov <- makeFEdummies(mdatapov$year)[,-1]
fydumpov <- makeFEdummies(fdatapov$year)[,-1]
